...
function BuyAndSellPage() {
  ...
  return (
    <Layout>
      <div className="relative min-h-screen bg-[#f4f8fc]">
        {/* ── Hero: reference layout (copy left, devices right) ── */}
        <section className="relative overflow-hidden border-b border-[#dbeafe]/80 bg-gradient-to-br from-[#f0f7ff] via-white to-[#e8f4ff]">
            ...
            <Reveal>
              <div>
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        mode === tab
                          ...
                          : "border border-slate-300 bg-white text-slate-800 hover:border-[#0056b3]/40 hover:bg-slate-50",
                      )}
                    >
                      {tab.toUpperCase()}
                    </button>
                ...
                <motion.p
                  ...
                  className="mt-8 max-w-lg text-slate-600 leading-relaxed"
                >
                  <span className="block font-semibold text-[#0f3b6f]">{modeCopy[mode].headline}</span>
                  {modeCopy[mode].body}
                </motion.p>
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  ...
                  <Button asChild variant="outline" className="rounded-lg border-slate-300 bg-white hover:bg-slate-50">
          ...
          <div className="mx-auto max-w-[1280px] px-4">
            <div className="grid items-stretch gap-8 lg:grid-cols-[1fr_360px] xl:grid-cols-[1fr_400px]">
              <div className="flex flex-col">
                <Reveal>
                  <div className="mb-10">
                    <h2 className="text-[2.2rem] font-extrabold tracking-tight text-[#0f1f47] md:text-[2.75rem]">
                      <span className="text-[#0056b3]">Buy</span> <span className="text-slate-900">&</span> <span className="text-[#10b981]">Sell</span> with Confidence
                    </h2>
                    <p className="mt-4 max-w-2xl text-[17px] text-slate-500">
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  {categories.map((cat, i) => (
                    <Reveal key={cat.title} delay={i * 0.06} className="h-full">
                      <Card className="group flex h-full flex-col overflow-hidden rounded-[2rem] border-0 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
                        <div className={`relative h-[160px] overflow-hidden ${cat.bg}`}>
                          <img
                            src={cat.img}
                            alt={cat.title}
                            loading="lazy"
                        ...
                        <div className="flex flex-1 flex-col p-5">
                          ...
                          <p className="mt-1 flex-1 text-[13px] text-slate-500">{cat.subtitle}</p>
            ...
            {/* Trust bar */}
            <Reveal delay={0.2}>
              <div className="mt-16 flex flex-col items-center justify-between gap-6 border-t border-slate-200 pt-8 sm:flex-row sm:gap-4 lg:mt-24 lg:pt-10">
                <div className="flex flex-wrap items-center justify-center gap-6 text-[15px] font-semibold text-slate-600 lg:gap-10">
        ...
        {/* ── How it works ── */}
        <section className="relative overflow-hidden py-20 md:py-24 bg-[#f4f8fc]">
          <SectionBackdrop wash="bg-[#f4f8fc]/80" />
          <div className="relative z-10 mx-auto max-w-7xl px-4">
            <Reveal>
              <div className="mb-12 text-center">
                <p className="text-xs font-bold uppercase tracking-[0.28em] text-[#0056b3]"></p>
                <h2 className="mt-3 text-3xl font-bold text-[#0f3b6f] md:text-4xl">How it works</h2>
                <p className="mx-auto mt-3 max-w-2xl text-slate-600">
                  Whether you&apos;re <span className="font-semibold text-[#047857]">buying, selling or exchanging</span> — the whole process takes just a few minutes in store.
                </p>
              </div>
            </Reveal>
              ...
              {steps.map((step, i) => (
                <Reveal key={step.n} delay={i * 0.08} className="h-full">
                  <Card className={themedCard(i, "rounded-2xl p-6")}>
                    <div className="mb-4 inline-flex h-10 w-10 items-center justify-center rounded-full bg-[#def7ec] text-sm font-bold text-[#047857]">
                      {step.n}
                    </div>
        ...
        <section className="relative bg-white py-16 md:py-20">
          <div className="mx-auto max-w-7xl px-4">
            <div className="grid gap-10 rounded-[2rem] border border-[#b3d9ff]/50 bg-gradient-to-br from-[#f0f7ff] to-white p-8 md:grid-cols-2 md:p-12">
              <Reveal>
                <Clock className="mb-4 h-9 w-9 text-[#047857]" />
                <p className="text-xs font-bold uppercase tracking-[0.28em] text-[#047857]">Opening hours</p>
                <h2 className="mt-2 text-3xl font-bold text-[#0f3b6f]">Visit us today</h2>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {hours.map((item, i) => (
                  <Reveal key={item.day} delay={i * 0.05}>
                    <div className="rounded-xl border border-[#b3d9ff]/60 bg-white p-4">
                      <div className="text-xs font-semibold uppercase tracking-wider text-slate-500">{item.day}</div>
                      <div className="mt-1 text-lg font-semibold text-[#047857]">{item.time}</div>
                    </div>
                  </Reveal>
                ))}
              </div>
        ...
        {/* ── Final CTA ── */}
        <section className="relative overflow-hidden py-20 bg-[#f4f8fc]">
          <div className="mx-auto max-w-4xl px-4">
            <Reveal>
              <Card className="relative overflow-hidden rounded-[2rem] border border-[#b3d9ff]/60 bg-white p-10 text-center shadow-lg md:p-14">
                <Sparkles className="mx-auto mb-4 h-6 w-6 text-[#10b981]" />
                <h2 className="text-3xl font-bold text-[#0f3b6f] md:text-4xl">Ready to buy or sell?</h2>
                ...
                <div className="mt-8 flex flex-wrap justify-center gap-4">
                  ...
                  <Button asChild size="lg" variant="outline" className="rounded-lg border-2 border-[#0056b3] text-[#0056b3] hover:bg-[#f0f7ff]">
