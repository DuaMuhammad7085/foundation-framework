...
import { CONTACT_IMAGES } from "@/assets/contact-images";
  ...
  head: () => ({
    meta: [
      { title: "Contact — Fixora Smart Repair" },
      {
        name: "description",
        content:
          "Contact Fixora for repair quotes, store directions, WhatsApp support and more. We're here Mon–Sun 10 AM – 8 PM.",
      },
    ],
  }),
...
const contactChannels = [
  {
    ...
    note: "Call for urgent repairs & quotes",
  },
  {
    icon: Mail,
    title: "Email",
    value: "support@fixora.com",
    href: "mailto:support@fixora.com",
    note: "We reply within 1 business hour",
  },
  {
    icon: MapPin,
    title: "Visit us",
    value: "123 Tech Street, Mega City — 400001",
  ...
  },
  {
    ...
    note: "Same-day repairs available",
  },
  {
    icon: MessageCircle,
    title: "WhatsApp",
  ...
  },
  {
    ...
    note: "Tips, offers & repair updates",
  },
];

const helpTopics = [
  { icon: Wrench, title: "Book a repair", text: "Schedule pickup or walk-in service.", to: "/book" as const },
  { icon: Headphones, title: "Track your device", text: "Check real-time repair status.", to: "/profile" as const },
  { icon: HelpCircle, title: "Browse FAQ", text: "Warranty, pricing & turnaround answers.", to: "/faq" as const },
];

/** Soft icon tile — avoids heavy blue blocks on white sections */
const iconTile =
    ...
    <Layout>
      ...
      <section className="relative overflow-hidden text-primary-foreground min-h-[380px] md:min-h-[420px]">
        <div
          ...
          style={{ backgroundImage: `url('${CONTACT_IMAGES.heroBg}')` }}
        />
        <GradientBackdrop variant="hero" className="opacity-88" />
        <div className="absolute inset-0 bg-gradient-to-r from-slate-950/80 via-slate-900/45 to-slate-900/20 pointer-events-none" />

        <div className="relative max-w-7xl mx-auto px-4 py-16 md:py-22 flex flex-col justify-center min-h-[380px] md:min-h-[420px]">
          <Reveal>
            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-sm text-xs uppercase tracking-widest mb-5">
              <Sparkles className="w-3.5 h-3.5" /> Contact Fixora
            </span>
            ...
            <p className="mt-5 text-lg text-white/85 max-w-xl leading-relaxed">
              Quotes, directions, warranty questions — reach us any way you prefer. Our team is ready to help.
            </p>
          </Reveal>
          <Reveal delay={0.12} className="mt-8 flex flex-wrap gap-3">
            <Button
              asChild
              size="lg"
              className="rounded-full bg-white text-slate-900 hover:bg-white/90 shadow-md border-0"
            >
            ...
            <Button
              ...
              size="lg"
              variant="outline"
              className="rounded-full border-white/35 text-white bg-white/10 hover:bg-white/15"
            >
          ...
          <Stagger className="grid sm:grid-cols-3 gap-4">
            {helpTopics.map((h) => (
              <StaggerItem key={h.title}>
                <Link
                  to={h.to}
                  className="group flex gap-4 p-5 rounded-2xl border border-border/80 bg-card hover:shadow-card hover:border-primary/25 transition-all"
                >
              ...
              <StaggerItem key={c.title}>
                <Card className="p-6 h-full bg-card hover:shadow-card hover:-translate-y-0.5 transition-all group border-border">
                  ...
                  {c.href ? (
                    <a
                      href={c.href}
                      className="font-semibold mt-1 block text-lg text-foreground hover:underline underline-offset-4 transition-colors"
                    >
                      {c.value}
                    </a>
                  ) : (
          ...
          <Reveal className="lg:col-span-3">
            <Card className="p-8 md:p-10 border-border shadow-card overflow-hidden relative bg-card">
              ...
              <p className="text-muted-foreground mb-8 relative">
                Fill out the form and we&apos;ll get back to you within one business hour — usually much sooner.
              </p>
              <form
                onSubmit={(e) => {
                  e.preventDefault();
              ...
              >
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input id="cphone" type="tel" className="mt-1.5" placeholder="+91 …" />
                  </div>
                </div>
                <div>
            ...
            <Reveal delay={0.08}>
              <Card className="p-8 border border-[#25D366]/25 bg-[#25D366]/[0.06] shadow-card">
                ...
                <p className="text-muted-foreground text-sm leading-relaxed mb-6">
                  Send photos of the damage, get instant quotes, and book pickup — all in one chat.
                </p>
                <Button
                  asChild
                  className="w-full rounded-full bg-[#25D366] text-white hover:bg-[#20bd5a] border-0 shadow-sm"
                >
            ...
            <Reveal delay={0.12}>
              <Card className="p-8 border-border/80">
                ...
                <ImagePlaceholder
                  ...
                  label="Store map / exterior"
                  hint="contact-map.jpg in public/contact/"
                  aspect="video"
                />
                <p className="text-sm text-muted-foreground mt-4 flex items-start gap-2">
                  <MapPin className="w-4 h-4 text-muted-foreground shrink-0 mt-0.5" />
                  123 Tech Street, Mega City, India — 400001
                </p>
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <p className="text-muted-foreground text-sm leading-relaxed">
                  Bring your device to our Mega City store for a free diagnosis. Most repairs are completed the
                  same day while you wait or shop nearby.
                </p>
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                {[
                  { label: "Weekdays", time: "10 AM – 8 PM" },
                  { label: "Weekends", time: "10 AM – 8 PM" },
                  { label: "Avg. wait", time: "Under 15 min" },
                  { label: "Parking", time: "Free on-site" },
                ].map((item) => (
                  <div
                    key={item.label}
                    className="rounded-2xl bg-muted/50 border border-border p-4 text-center"
                  >
